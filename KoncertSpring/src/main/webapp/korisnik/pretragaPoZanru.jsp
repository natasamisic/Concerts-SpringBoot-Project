...
<body id="myPage" data-spy="scroll" data-target=".navbar"
	...
	<div class="container text-center">
		<form action="/Koncert/user/getKoncertePoZanru" method="get">
			Izaberite žanr: <select name="zanr">
				<c:forEach items="${zanrovi }" var="z">
					<option value="${z.idzanr }">${z.naziv }</option>
				</c:forEach>
			</select> <input class="btn" type="submit" value="Prikaži">
		</form>
		<br>
		<br>
			...
			<table class="table table-bordered">
				...
				<c:forEach items="${koncerti }" var="k">
					...
					<tr>
						...
						<td>${k.cena }</td>
						<td>${k.opis }</td>
						<td><a href="/Koncert/user/prikaziKoncert?idK=${k.idKoncert}" class="btn">Prikaži</a></td>
					</tr>
