...
<body id="myPage" data-spy="scroll" data-target=".navbar" data-offset="50">
	...
	<div class="container text-center">
		<c:if test="${!empty mojikomentari }">
			<table class="table table-bordered">
				<tr>
					<th>Koncert</th>
					<th>Komentar</th>
				</tr>
				<c:forEach items="${mojikomentari }" var="k">
					<tr>
					<td>${k.koncert.izvodjac }</td>
					<td>${k.tekst }</td>
					<td><form action="/Koncert/user/obrisiKomentar?idK=${k.idKomentar}" method="post"><button class="btn" type="submit">Obriši komentar</button></form></td>
				</tr>
				</c:forEach>
			</table>
		</c:if>
